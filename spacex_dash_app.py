...
# Callback để cập nhật nội dung dựa trên giá trị đã chọn
@app.callback(
...
def update_output(selected_site):
    if selected_site == 'ALL':
        return "You have selected all sites"
    else:
        return f"You have selected {selected_site}"

# TASK 4: Add a callback function for `site-dropdown` and `payload-slider` inputs
@app.callback(
...
def update_scatter_chart(selected_site, payload_range):
    ...
    fig = px.scatter(
        ...
        title=f'Payload vs. Success for site: {selected_site}' if selected_site != 'ALL' else 'Payload vs. Success for All Sites',
